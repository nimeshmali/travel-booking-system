...
const TourPackageForm = () => {
	...
	const addDateRange = () => {
		setFormData((prevState) => ({
			...prevState,
			availableDates: [...prevState.availableDates, { startDate: "", endDate: "" }],
		}));
	};
		...
		<div className='container mx-auto px-8 py-8'>
			...
			<div className='bg-white shadow-md rounded-lg p-6'>
				<form onSubmit={handleSubmit}>
					...
					{/* Image Upload */}
					<div className='mb-4'>
						<label className='block text-gray-700 font-semibold mb-2'>Package Image</label>
						<input
							type='file'
							accept='image/*'
							...
							required
						/>
						{formData.image && <img src={formData.image} alt='Preview' className='mt-4 w-full h-48 object-cover rounded-lg' />}
					</div>

					{/* Available Dates */}
					<div className='mb-4'>
						<label className='block text-gray-700 font-semibold mb-2'>Available Dates</label>
						{formData.availableDates.map((date, index) => (
							<div key={index} className='grid grid-cols-3 gap-4 items-end mb-4'>
								...
								{index > 0 && (
									<button type='button' onClick={() => removeDateRange(index)} className='text-red-600 font-semibold hover:underline'>
										Remove
									</button>
								)}
							</div>
					...
					<button type='button' onClick={addDateRange} className='text-blue-500 font-semibold hover:underline'>
