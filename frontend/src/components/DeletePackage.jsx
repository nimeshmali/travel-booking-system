...
const DeletePackage = () => {
	...
	const handleDeletePackage = async () => {
		...
		try {
			...
			const packageToDelete = packages.find((pkg) => pkg.title === selectedPackage);
			...
			await packageService.deletePackage(packageToDelete._id);
	...
	return (
		<div className='container mx-auto px-8 py-8'>
			<div className='bg-gradient-to-r from-blue-600 to-purple-700 text-white p-8 rounded-xl shadow-lg mb-8'>
				<h1 className='text-4xl font-bold mb-4 tracking-tight'>Manage Travel Packages</h1>
				<p className='text-xl opacity-90'>Carefully select and remove travel packages from your catalog</p>
			</div>

			<div className='bg-white shadow-md rounded-lg p-6'>
				<div className='mb-4'>
					<label htmlFor='package-select' className='block text-gray-700 font-semibold mb-2'>
			...
			{selectedPackage && (
				<div className='mt-8 bg-gray-100 rounded-lg p-6'>
					<h2 className='text-2xl font-semibold mb-4 text-gray-800'>Package Preview</h2>
					{(() => {
						const selectedPkgDetails = packages.find((pkg) => pkg.title === selectedPackage);
						return selectedPkgDetails ? (
							<div className='grid md:grid-cols-2 gap-4'>
								<div>
									...
									<p>
										<strong>Description:</strong> {selectedPkgDetails.description}
									</p>
								</div>
								<div>
									{selectedPkgDetails.image && (
										<img src={selectedPkgDetails.image} alt={selectedPkgDetails.title} className='w-full h-48 object-cover rounded-md' />
									)}
								</div>
							</div>
						) : null;
