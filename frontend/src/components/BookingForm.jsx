...
const PackageBookingPage = () => {
	...
	const handleSubmit = async (e) => {
		e.preventDefault();
		try {
			const response = await packageService.bookPackage(packageId, formData);
			console.log("response ", response);

			alert(response.message || "Booking confirmed successfully!");
			navigate("/");
		} catch (error) {
			const errorMessage = error.response?.data?.error || "Failed to book package. Please try again.";
			alert(errorMessage);
		}
	};
	...
	const startDate = packageDetails.availableDates && packageDetails.availableDates.length > 0 ? packageDetails.availableDates[0].startDate : null;

	const endDate = packageDetails.availableDates && packageDetails.availableDates.length > 0 ? packageDetails.availableDates[0].endDate : null;
			...
			<div className='lg:w-1/2'>
				<div className='bg-white shadow-2xl rounded-xl h-full transform transition-all '>
					...
					<div className='p-6 mt-3'>
						<h1 className='text-3xl font-bold text-gray-600 mb-4'>{packageDetails.title}</h1>

						<p className='text-gray-400 font-semibold text-lg leading-relaxed mb-6'>{packageDetails.description}</p>
						...
						<div className='grid grid-cols-2 gap-4 text-gray-700 mt-10'>
							{startDate && (
								<div className='flex items-center space-x-2'>
									...
									<div>
										<h3 className='font-semibold text-sm text-gray-600'>Start Date</h3>
							...
							{endDate && (
								<div className='flex items-center space-x-2'>
									<CalendarIcon className='w-7 h-7 text-blue-500' />
									<div>
										<h3 className='font-semibold text-sm text-gray-600'>End Date</h3>
							...
							<div className='flex items-center space-x-2'>
								...
								<div>
									<p className='text-green-600 font-semibold text-2xl'>{packageDetails.price.toLocaleString()}</p>
								</div>
							</div>
							...
							{startDate && endDate && (
								<div className='flex items-center space-x-2'>
									...
									<div>
										<h3 className='font-semibold text-sm text-gray-600'>Duration</h3>
			...
			<div className='lg:w-1/2 mx-auto'>
				<div className='bg-white shadow-lg rounded-lg p-8 transform transition-all hover:shadow-xl'>
					...
					<form onSubmit={handleSubmit} className='space-y-6'>
						...
						<div>
							<label htmlFor='numberOfTravelers' className='block text-md font-medium text-gray-600'>
								Number of Travelers
							</label>
						...
						<div>
							<label htmlFor='specialRequests' className='block text-md font-medium text-gray-600'>
								Special Requests
							</label>
							<textarea
								id='specialRequests'
